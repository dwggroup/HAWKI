...
if (file_exists(".env")){
	...
	$gpt_version = isset($env) ? $env["GPT_VERSION"] : getenv("GPT_VERSION");
	$logo_image_filename = isset($env) ? $env["LOGO_IMAGE_FILENAME"] : getenv("LOGO_IMAGE_FILENAME");
	
	
	$_SESSION['system_prompt'] = $system_prompt;
	$_SESSION['gpt_version'] = $gpt_version;
	$_SESSION['logo_image_filename'] = $logo_image_filename;
}
...
<div class="wrapper">
  <div class="sidebar">
	<div class="logo" onclick="load(this, 'chat.php')">
	 <img src="/img/<?= $_SESSION['logo_image_filename'] ?>" alt="HAWK Logo" width="150px"><!-- TODO ordentliches Logo als svg -->
	</div>
	<div class="menu">
		<details>
			<summary>
	  ...
	  <div class="menu-item" onclick="load(this, 'chat.php')">
...
<div class="modal" onclick="modalClick(this)" id="gpt4"> 
	<div class="modal-content">
		<h2>GPT 3.5</h2>
		<p>Es wird das Model gpt-4-0125-preview verwendet.   
		</p>
		<button>Bestätigen</button>
	</div>
</div>
...
<script>
	...
	async function request(){
		...
		const requestObject = {};
		requestObject.model = '<?= $_SESSION['gpt_version'] ?>';
